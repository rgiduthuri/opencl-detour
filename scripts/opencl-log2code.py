...
commandBuffer = []
commandBufferSaved = []
def saveCommandBuffer(v):
    if v[0] == 'copy-buffer':
        cmd = ['copy', v[1], v[2], v[3], v[4], v[5]]
    else:
        arg = []
        for i in v[0][2]:
            arg.append(i)
        cmd = ['kern', v[0][1], v[0][0], arg, v[1]]
    ...
    if (skipTillToken != '') and ('OPENCL-TRACE: ' + skipTillToken in line):
        ...
        if size == 8:
            if param[5] in dictBuffer:
                buff = param[5]
                v = (arg, 'buf', buff, lineno)
                if not kern in dictBufferUse[buff]:
                    dictBufferUse[buff].append(kern)
            else:
                v = (arg, 'data', [ int(param[5], 0) & 0xffffffff, int(param[5], 0) >> 32], lineno)
        else:
            v = []
            for i in param[5:]:
                v.append(int(i, 0))
        ...
        enqueue = [dictKernel[kern], (dims, origin, globalWork, localWork, lineno)]
        saveCommandBuffer(enqueue)
        ...
        if len(commandBuffer) > 0:
            commandBufferSaved = commandBuffer
            commandBuffer = []

# get number of kernel and buffer objects
commandBuffer = commandBufferSaved
countKern = 0
countCopy = 0
bufferList = []
programList = []
bufferUsage = []
for ic, cmd in enumerate(commandBuffer):
    if cmd[0] == 'copy':
        countCopy = countCopy + 1
        bufUse = [cmd[1], cmd[2]]
    elif cmd[0] == 'kern':
        countKern = countKern + 1
        prog = cmd[1]
        if not prog in programList:
            found = prog
            vprog = dictProgram[prog]
            for p in programList:
                vp = dictProgram[p]
                if (vp[0] == vprog[0]) and (vp[1] == vprog[1]) and (vp[4] == vprog[4]):
                    found = p
            if found != prog:
                commandBuffer[ic][1] = found
            else:
        ...
        for a in cmd[3]:
            if a[1] == 'buf':
                bufUse.append(a[2])
    for buf in bufUse:
        if not buf in bufferList:
            bufferList.append(buf)
    bufferUsage.append(bufUse)
countBuffer = len(bufferList)
countProgram = len(programList)
countCommand = len(commandBuffer)
...
fpC.write('    cl_mem buf[' + str(countBuffer) + '];\n')
fpC.write('    cl_kernel kern[' + str(countKern) + '];\n')
fpC.write('};\n')
fpC.write('\n')
...
for i, buff in enumerate(bufferList):
    v = dictBuffer[buff]
    if len(v[2]) > 0:
...
fpC.write('{\n')
fpC.write('    return ' + str(countBuffer) + ';\n')
fpC.write('}\n')
fpC.write('\n')
...
for vk in commandBuffer:
    if vk[0] == 'kern':
        v = vk[3] # (name, programHandle, [(index,'buf'|'data',bufferHandle|<byte-array>), ...])
        for a in v:
            if a[1] == 'buf':
                fpC.write('    ERRCHK_ERNUL(clSetKernelArg(job->kern[%d], %d, sizeof(cl_mem), &job->buf[%d])); // line#%d\n' % (ik, a[0], bufferList.index(a[2]), a[3]))
            elif a[1] == 'data':
...
ik = 0
ic = 0
for vk in commandBuffer: # (kernelHandle, dims, origin, globalWork, localWork)
    if vk[0] == 'kern':
        dims = vk[4][0]
        origin = vk[4][1]
        globalWork = vk[4][2]
        ...
        if localWork[0] >= 0:
            ...
            fpC.write(' NULL,')
        fpC.write(' 0, NULL, NULL), %d); } // line#%d\n' % (ic, code))
        ik = ik + 1
    elif vk[0] == 'copy':
        buf1 = bufferList.index(vk[1])
        buf2 = bufferList.index(vk[2])
        soffset = vk[3]
        doffset = vk[4]
        bufsize = vk[5]
...
print('OK: job allocates %d buffers (%d bytes allocation with %d bytes pre-initialized)' % (countBuffer, totalBufferAlloc, totalBufferInit))
print('OK: job uses %d kernels created from %d programs (unique %d source + %d binary) with %d bytes of code size' % (countKern, countProgram, numProgSrc, numProgBin, totalProgSrc + totalProgBin))
if countCopy > 0:
    print('OK: job uses %d buffer copies' % (countCopy))
...
fpM.write('{\n')
fpM.write('    const char * usage = "Usage: job [[read:bufIndex,fileName]|[write:bufIndex,fileName]|[compare:bufIndex,fileName]|[iter:numIter]]";\n')
fpM.write('    int err;\n')
fpM.write('\n')
...
fpM.write('        else if(strncmp(argv[i], "write:", 6) == 0) ;\n')
fpM.write('        else if(strncmp(argv[i], "compare:", 8) == 0) ;\n')
fpM.write('        else { printf("ERROR: invalid option: %s\\n%s\\n", argv[i], usage); return -1; }\n')
fpM.write('    }\n')
fpM.write('\n')
fpM.write('    begin = std::chrono::steady_clock::now();\n')
...
fpM.write('            if(index >= ' + jobName + 'GetNumBuffer()) { printf("ERROR: invalid buffer index to write: %ld\\n", index); return -1; }\n')
...
fpM.write('        }\n')
fpM.write('        else if(!strncmp(argv[i], "compare:", 8)) {\n')
fpM.write('            size_t index = 0;\n')
fpM.write('            char fileName[1024] = { 0 };\n')
fpM.write('            if(sscanf(&argv[i][8], "%ld,%s", &index, fileName) != 2) { printf("ERROR: invalid argument: %s\\n%s\\n", argv[i], usage); return -1; }\n')
fpM.write('            if(index >= ' + jobName + 'GetNumBuffer()) { printf("ERROR: invalid buffer index to compare: %ld\\n", index); return -1; }\n')
fpM.write('            FILE * fp = fopen(fileName, "rb"); if(!fp) { printf("ERROR: unable to open: %s\\n", fileName); return -1; }\n')
fpM.write('            cl_mem mem = ' + jobName + 'GetMem(job, index);\n')
fpM.write('            size_t size = 0;\n')
fpM.write('            ERRCHK_ERERR(clGetMemObjectInfo(mem, CL_MEM_SIZE, sizeof(size), &size, NULL));\n')
fpM.write('            void * q = malloc(size);\n')
fpM.write('            if(fread(q, 1, size, fp) != size) { printf("ERROR: unable to read %ld bytes from %s\\n", size, fileName); return -1; }\n')
fpM.write('            fclose(fp);\n')
fpM.write('            void * p = clEnqueueMapBuffer(cmdq, mem, CL_TRUE, CL_MAP_READ, 0, size, 0, NULL, NULL, &err);\n')
fpM.write('            ERRCHK_ERERR(err);\n')
fpM.write('            ERRCHK_ERERR(clFinish(cmdq));\n')
fpM.write('            float avgerr = 0, maxerr = 0;\n')
fpM.write('            for(size_t i = 0; i < size/4; i++) {\n')
fpM.write('              float d = ((float *)p)[i] - ((float *)q)[i];\n')
fpM.write('              d = (d < 0) ? -d : d;\n')
fpM.write('              maxerr = (d > maxerr) ? d : maxerr;\n')
fpM.write('              avgerr += d * d;\n')
fpM.write('            }\n')
fpM.write('            ERRCHK_ERERR(clEnqueueUnmapMemObject(cmdq, mem, p, 0, NULL, NULL));\n');
fpM.write('            ERRCHK_ERERR(clFinish(cmdq));\n')
fpM.write('            free(q);\n')
fpM.write('            avgerr /= (float)(size/4);\n')
fpM.write('            printf("OK: compare mem[%ld] with %s (%ld bytes) -- maxerr:%g avgerr:%g\\n", index, fileName, size, maxerr, avgerr);\n')
fpM.write('        }\n')
fpM.write('    }\n')
fpM.write('\n')
